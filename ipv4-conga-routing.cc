...
{
  std::map<uint32_t, std::map<uint32_t, std::pair<Time, std::vector<uint32_t> > > >::iterator itr =
      m_congaToLeafTable.find(leafId);
  if (itr != m_congaToLeafTable.end ())
  {
    std::vector<uint32_t> temp;
    temp.push_back(50);
    (itr->second)[port] = std::make_pair(Simulator::Now (), temp);
  }
  else
  {
    std::map<uint32_t, std::pair<Time, std::vector<uint32_t> > > newMap;
    std::vector<uint32_t> temp;
    temp.push_back(50);
    newMap[port] = std::make_pair(Simulator::Now (), temp);
                           ...
                           LocalDeliverCallback lcb, ErrorCallback ecb)
{
  ...
  if (m_isLeaf)
  {
    ...
    if (!found)
    {
      ...
      std::map<uint32_t, std::map<uint32_t, std::pair<Time, std::vector<uint32_t> > > >::iterator
      ...
      for ( ; routeEntryItr != routeEntries.end (); ++routeEntryItr)
      {
        ...
        */
        std::map<uint32_t, std::pair<Time, std::vector<uint32_t> > >::iterator remoteCongestionItr =
    ...
    else
    {
      ...
      {
        std::map<uint32_t, std::map<uint32_t, std::pair<Time, std::vector<uint32_t> > > >::iterator toLeafItr =
        ...
        else
        {
          ...
          temp.push_back(ipv4CongaTag.GetFbMetric ());
          std::map<uint32_t, std::pair<Time, std::vector<uint32_t> > > newMap;
          newMap[ipv4CongaTag.GetFbLbTag ()] = std::make_pair(Simulator::Now (), temp);
...
Ipv4CongaRouting::AgingEvent ()
{
    ...
    {
      std::map<uint32_t, std::pair<Time, std::vector<uint32_t> > >::iterator innerItr =
        (itr->second).begin ();
